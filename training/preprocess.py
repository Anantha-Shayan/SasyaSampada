...
if __name__ == "__main__":
    ...
    file_path = DATASETS_DIR / "ICRISAT-District Level Data.csv"

    cleaned_df, district_crop_map, crop_district_map = generate_crop_mappings(file_path)
    cleaned_df.to_csv(PROCESSED_DIR / "icrisat_cleaned.csv", index=False)

    # Save both JSONs
    with (MODEL_ASSETS_DIR / "district_crop_map.json").open("w", encoding="utf-8") as f:
        json.dump({f"{k[0]}|{k[1]}": v for k, v in district_crop_map.items()}, f, indent=2)

    with (MODEL_ASSETS_DIR / "crop_district_map.json").open("w", encoding="utf-8") as f:
        json.dump(crop_district_map, f, indent=2)

    print(f"✅ Saved crop mappings to {MODEL_ASSETS_DIR}")
